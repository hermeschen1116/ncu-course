...
class DataPreprocess:
	...
	def get_dataset(self, tokenizer: Any) -> QADataset:
		...
		encoding_set = tokenizer(self.raw_data['question'], self.raw_data['reference'], truncation=self.__tokenizer_prams['truncation'],
			padding=self.__tokenizer_prams['padding'], return_tensors=self.__tokenizer_prams['return_tensors'])
...
class QAModel(torch_lightning.LightningModule):
	...
	def forward(self, input_batch):
		bert_output = self.bert(input_ids=input_batch['input_ids'], attention_mask=input_batch['attention_mask'], token_type_ids=input_batch['token_type_ids'],
			return_dict=True)
